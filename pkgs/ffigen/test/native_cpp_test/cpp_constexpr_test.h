...
// `constexpr` variables and `static constexpr` data members are parsed as
// const globals, named by their enclosing scopes.
...
// A mutable namespace-scoped variable. It is read through a mangled symbol, so
// it must not be surfaced as a binding.
extern int nsMutable;
}  // namespace ns
